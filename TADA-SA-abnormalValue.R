...
cutq <- 0.95
trunc_obj <- TransportHealth::trunc("quantile", cutq)

n_study <- 200     

outlierType  <- c(NULL, "X2_flip", "X3_extreme", "both")   # "X3_extreme" or "X2_flip" or both or NULL
outlierFrac  <- c(0.01, 0.05)                              # abv proportion (1%)
outlierMult  <- 3                                          # X3_extreme SD:  tp_obs$X3[idx] <- tp_obs$X3[idx] + outlier.mult * sd3

ABV_Scenarios <- bind_rows(
  tibble(
    ABV_Type               = "None",
    X2_Flips_Prop          = NA_real_,
    X3_Extremes_Prop       = NA_real_,
    X3_Extremes_Multiplier = NA_real_
  ),
  
  expand.grid(
    ABV_Type      = "X2_flip",
    X2_Flips_Prop = outlierFrac,
  ...
  expand.grid(
    ABV_Type               = "X3_extreme",
    X3_Extremes_Prop       = outlierFrac,
    X3_Extremes_Multiplier = outlierMult,
    stringsAsFactors       = FALSE
  ) %>%
    mutate(
      X2_Flips_Prop = NA_real_
  ...
  select(ABV_Type, X2_Flips_Prop, X3_Extremes_Prop, X3_Extremes_Multiplier) %>%
  slice(-c(7, 8))
...
for (sc in seq_len(nrow(ABV_Scenarios))) {
  scen <- ABV_Scenarios[sc, ]
  ...
  print(paste0(
    "[ Sensitivity Analysis - Abnormal Values in Target Data ] ",
    "ABV Type = ", scen$ABV_Type,
    ", X2_flip_prop = ", scen$X2_Flips_Prop,
    ", X3_extreme_prop = ", scen$X3_Extremes_Prop,
    ", X3_extreme_mult = ", scen$X3_Extremes_Multiplier
  ))
  ...
  for (rep in seq_len(nRep)) {
    ...
    testData <- generateTestDataTADA_TTE_ABV(nStudy = n_study, base_risk_cen = base_risk_cen,
                                             ABV_Type                = scen$ABV_Type,
                                             X2_Flips_Prop           = scen$X2_Flips_Prop,
                                             X3_Extremes_Prop        = scen$X3_Extremes_Prop,
                                             X3_Extremes_Multiplier  = scen$X3_Extremes_Multiplier)
    
    studyData            <- testData$studyData[[rr]][, -1]                   # study sample IPD
    rownames(studyData)  <- seq_len(nrow(studyData))
    aggregateData        <- testData$aggregateTargetData_abv[[rr]]           # target sample AgD 
    targetIPD            <- testData$IPDTargetPopulationData_abv[[rr]][,-1]  # target population IPD
  ...
  summary_list[[length(summary_list) + 1]] <- data.frame(
    ABV_Type               = scen$ABV_Type,
    X2_Flips_Prop          = ifelse(is.na(scen$X2_Flips_Prop), "-", paste0(scen$X2_Flips_Prop * 100, "%")),
    X3_Extremes_Prop       = ifelse(is.na(scen$X3_Extremes_Prop), "-", paste0(scen$X3_Extremes_Prop * 100, "%")),
    X3_Extremes_Multiplier = ifelse(is.na(scen$X3_Extremes_Multiplier), "-", paste0(scen$X3_Extremes_Multiplier, "x")),
