...
# sensitivity analysis: study sample size n_study
nStudy <- c(300, 500, 800, 1000)     # study sample size
...
for (n_study in nStudy) {
  ...
  print(paste0("[ Sensitivity Analysis - Study Sample Size ] The study sample size is: ", n_study))
  ...
  for (rep in seq_len(nRep)) {
    ...
    # Bootstrap: MoM weights == final weights
    ...
    HR_cen_Boot <- foreach(
      ...
      tryCatch({
        ...
        repeat {
          ...
          studyData_long_event_boot <- studyData_long_event_boot %>%
            mutate(
              censoring_survival   = pmax(exp(-hazard * exp(linear_predictor)), 1e-6)
            )
