...
cutq_final <- 0.95
trunc_obj <- if (is.na(cutq_final)) NULL else TransportHealth::trunc("quantile", cutq_final)
...
for (rr in 1:10) {
  ...
  print(paste0("[ Scenario ", rr, " ] The current study sample size is: ", n_study))
  
  for (rep in seq_len(nRep)) {
    
    print(paste0("---- ", rep, "-th replicate starts ----"))
    set.seed(20250425 + rep)
    
    testData <- generateTestDataTADA_TTE(nStudy = n_study, 
    ...
    cenPropAll <- c(cenPropAll, mean(studyData$censor == 1))
    cenPropTrt <- c(cenPropTrt, mean(studyData[which(studyData$treatment == 1),]$censor == 1))
    cenPropCtrl <- c(cenPropCtrl, mean(studyData[which(studyData$treatment == 0),]$censor == 1))
    ...
    studyData_long_event <- studyData_long_event %>%
      ...
      mutate(
        ...
        final_weights = pmax(pmin(raw_final_wt, quantile(raw_final_wt, probs = cutq_final, na.rm = TRUE)), 1e-6)
      )
    ...
    HR_cen_Boot <- foreach(
      ...
      tryCatch({
        ...
        studyData_long_event_boot <- studyData_long_event_boot %>%
          mutate(baseline_survival = exp(-hazard))
        ...
        studyData_long_event_boot <- studyData_long_event_boot %>%
          mutate(final_weights_bootstrap = pmax(pmin(raw_final_wt_bootstrap, 
                                                quantile(raw_final_wt_bootstrap, probs = cutq_final, na.rm = TRUE)), 1e-6))
    ...
    # ------  pseudo true model  --------------------------------------------------------------------------------- #
    res_true <- coxph(Surv(eventTime) ~ treatment, data = targetIPD, robust = T, timefix = F)
  ...
  raw_data_list <- list(
    ...
    cenPropAll        = cenPropAll,
    cenPropTrt        = cenPropTrt,
    cenPropCtrl       = cenPropCtrl
  )
  # saveRDS(
  #   raw_data_list,
  #   file = file.path(sprintf("%gcen", cenProp), sprintf("raw_results_%gcen.rds", cenProp))
  # )
  
  TADA_Results_rr <- data.frame(
    Scenarios = rr,
    
    HR_censor = sprintf("%.3f (%.3f, %.3f)", HR_cen_average, CI_lower_cen_average, CI_upper_cen_average),
    HR_noncensor = sprintf("%.3f (%.3f, %.3f)", HR_noncen_average, CI_lower_noncen_average, CI_upper_noncen_average),
    HR_true = sprintf("%.3f (%.3f, %.3f)", trueHR_average, CI_lower_true_average, CI_upper_true_average),
    
    Bias_HR_censor = sprintf("%.3f (%.3f, %.3f)", bias_HR_cen,
                             unlist(Percentile_Lower_Bias_HR_censor), unlist(Percentile_Upper_Bias_HR_censor)),
    Bias_HR_noncensor = sprintf("%.3f (%.3f, %.3f)", bias_HR_noncen,
                                unlist(Percentile_Lower_Bias_HR_noncensor), unlist(Percentile_Upper_Bias_HR_noncensor)),
    
    Coverage_HR_censor = coverage_HR_cen,
    Coverage_HR_noncensor = coverage_HR_noncen,
    
    MonteCarlo_SE_cen = round(MonteCarlo_SE_cen, 3),
    Bootstrap_SE_cen = round(Bootstrap_SE_cen, 3), 
    
    MonteCarlo_SE_noncen = round(MonteCarlo_SE_noncen, 3),
    Bootstrap_SE_noncen = round(Bootstrap_SE_noncen, 3)
    
  )
  
  TADA_Results <- rbind(TADA_Results, TADA_Results_rr) 
...
# write.csv(
#   TADA_Results,
#   file      = file.path(sprintf("%gcen", cenProp), "TADA_Results.csv"),
#   row.names = FALSE
# )
